  ...
  return (
    <AppShell>
      <div className="space-y-6">
        ...
        {/* Header da Equipe */}
        <Card className="p-4 sm:p-6 bg-card/90 border shadow-xs">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <Card className="p-4 sm:p-5 bg-card/90 border shadow-xs">
            ...
            <div className="mt-2 w-full bg-muted rounded-full h-1.5 overflow-hidden">
              <div
                className={`h-full transition-all duration-300 ${
                  isFull ? "bg-amber-500" : occupancyPercent > 70 ? "bg-primary" : "bg-emerald-500"
                }`}
          ...
          <Card className="p-4 sm:p-5 bg-card/90 border shadow-xs">
            ...
            <div className={`text-2xl sm:text-3xl font-bold ${
              waitingCount > 0 ? "text-amber-600 dark:text-amber-400" : "text-foreground"
            }`}>
        ...
        {teamSummary?.agents && teamSummary.agents.length > 0 && (
          <div className="space-y-3">
            <h2 className="text-xs font-semibold tracking-wider uppercase text-muted-foreground">
              Operadores Alocados ({teamSummary.agents.length})
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
              {teamSummary.agents.map((agent) => {
                ...
                return (
                  <Card key={agent.id} className="p-3.5 bg-card/80 border shadow-xs space-y-2.5">
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="w-full bg-muted rounded-full h-1 overflow-hidden">
                        <div
                          className={`h-full transition-all duration-300 ${
                            isAgentFull ? "bg-amber-500" : "bg-emerald-500"
                          }`}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
          {/* Fila Ativa */}
          <Card className="w-full overflow-hidden shadow-xs border">
            <CardHeader className="flex flex-row items-center justify-between pb-3 bg-muted/20 border-b px-4 py-3.5">
              <div>
                ...
                <CardDescription className="text-[11px] text-muted-foreground mt-0.5">
                  Chamados atribuídos aos atendentes de {label}
                </CardDescription>
              </div>
            </CardHeader>
                    ...
                    <TableRow>
                      <TableCell colSpan={4} className="h-28 text-center text-muted-foreground">
                        <div className="flex flex-col items-center justify-center gap-1.5 text-xs py-3">
                          ...
                          <span>Nenhum atendimento em andamento nesta equipe.</span>
                        </div>
                      </TableCell>
                    </TableRow>
                  ) : (
                    teamActiveQueue.map((item) => {
            ...
            <CardContent className="p-0">
              <Table className="w-full">
                ...
                <TableBody>
                  ...
                  ) : teamWaitingQueue.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={3} className="h-28 text-center text-muted-foreground">
                        <div className="flex flex-col items-center justify-center gap-1.5 text-xs py-3">
                          <Inbox className="h-5 w-5 text-muted-foreground/40" />
                          <span>Nenhum chamado na fila de espera desta equipe.</span>
                        </div>
                      </TableCell>
                    </TableRow>
                  ) : (
                    teamWaitingQueue.map((item) => (
                      <TableRow key={item.id} className="hover:bg-muted/30 transition-colors">
                        <TableCell className="py-2.5 px-3 align-middle">
